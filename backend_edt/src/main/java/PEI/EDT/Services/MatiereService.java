...
public class MatiereService {
    ...
    public MatiereDto create(CreateMatiereDto dto) {
        ...
        String assignedType = dto.getTypeMatiere();
        if (user.getRole() == RoleUtilisateur.CHEF_DEP) {
            assignedType = "DEP";
        } else if (user.getRole() == RoleUtilisateur.CHEF_HE) {
            assignedType = "HE";
        } else if (user.getRole() == RoleUtilisateur.CHEF_ST) {
            assignedType = "ST";
        }
    ...
    public List<MatiereDto> list() {
        ...
        if (user.getRole() == RoleUtilisateur.CHEF_DEP) {
            matieres = matieres.stream().filter(m -> "DEP".equals(m.getTypeMatiere())).collect(Collectors.toList());
        } else if (user.getRole() == RoleUtilisateur.CHEF_HE) {
            matieres = matieres.stream().filter(m -> "HE".equals(m.getTypeMatiere())).collect(Collectors.toList());
        } else if (user.getRole() == RoleUtilisateur.CHEF_ST) {
            matieres = matieres.stream().filter(m -> "ST".equals(m.getTypeMatiere())).collect(Collectors.toList());
        }

        return matieres.stream().map(this::toDto).toList();
    }
