  ...
  return (
    <div className="w-full overflow-x-auto pb-2">
      <div className="min-w-[500px]">
        {/* Header Row */}
        <div className="grid grid-cols-[100px_repeat(6,1fr)] gap-3 mb-3">
          <div className="text-xs font-medium text-muted-foreground p-2 flex items-center justify-center">Horaires</div>
          {DAYS.map(day => (
            <div key={day} className="bg-slate-100 dark:bg-slate-800 rounded-md p-2 text-xs font-bold text-slate-600 dark:text-slate-300 text-center tracking-wide uppercase">{day.substring(0, 3)}</div>
          ))}
        </div>
        ...
        {SLOTS.map((slot, sIndex) => {
          ...
          return (
            <div key={slot} className="grid grid-cols-[100px_repeat(6,1fr)] gap-3 mb-3">
              {/* Time Label (Editable) */}
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="bg-white dark:bg-slate-800 rounded-md py-2 px-1 flex flex-col items-center justify-center hover:bg-slate-50 dark:hover:bg-slate-700 transition-all border border-slate-200 dark:border-slate-700 hover:border-primary/50 cursor-pointer shadow-sm">
              ...
              {DAYS.map((day, dIndex) => {
                ...
                return (
                  <DropdownMenu key={`${day}-${slot}`}>
                    <DropdownMenuTrigger asChild>
                      <button className={cn(
                        "h-full w-full rounded-md py-3 text-[11px] tracking-wider font-bold text-white shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer border border-transparent focus:ring-2 focus:ring-offset-1 focus:ring-offset-background",
                        getCellColor(type),
                        creneau?._dirty ? "ring-2 ring-yellow-400" : ""
                      )}>
                        {type}
                      </button>
                    </DropdownMenuTrigger>
